package models

import (
	"html"
	"strings"
	"time"

	"api-finalproject/utils/token"

	"golang.org/x/crypto/bcrypt"
	"gorm.io/gorm"
)

type (
	// customers
	Customer struct {
		ID        uint      `gorm:"primary_key" json:"id"`
		Name      string    `json:"name"`
		Username  string    `gorm:"not null;unique" json:"username"`
		Email     string    `gorm:"not null;unique" json:"email"`
		Password  string    `gorm:"not null;" json:"password"`
		CreatedAt time.Time `json:"created_at"`
		UpdatedAt time.Time `json:"updated_at"`
		Orders    []Order   `json:"-"`
	}
)

func VerifyPassword(password, hashedPassword string) error {
	return bcrypt.CompareHashAndPassword([]byte(hashedPassword), []byte(password))
}

func LoginCheck(username string, password string, db *gorm.DB) (Customer, string, error) {

	var err error

	u := Customer{}

	err = db.Model(Customer{}).Where("username = ?", username).Take(&u).Error

	if err != nil {
		return u, "", err
	}

	err = VerifyPassword(password, u.Password)

	if err != nil && err == bcrypt.ErrMismatchedHashAndPassword {
		return u, "", err
	}

	token, err := token.GenerateToken(u.ID)

	if err != nil {
		return u, "", err
	}

	return u, token, nil

}

func (u *Customer) SaveUser(db *gorm.DB) (*Customer, error) {
	//turn password into hash
	hashedPassword, errPassword := bcrypt.GenerateFromPassword([]byte(u.Password), bcrypt.DefaultCost)
	if errPassword != nil {
		return &Customer{}, errPassword
	}
	u.Password = string(hashedPassword)
	//remove spaces in username
	u.Username = html.EscapeString(strings.TrimSpace(u.Username))

	var err error = db.Create(&u).Error
	if err != nil {
		return &Customer{}, err
	}
	return u, nil
}
